...
private var mDefaultRect : Rect;
var mTouchZone : Rect = Rect(0, 0, 300, 300);
...
function Start () {
    ...
    {
        mTouchZone.width = 0.3 * Screen.width - background.pixelInset.width * 0.5;
        mTouchZone.height = 0.4 * Screen.height;
    }
    else
    {
        mTouchZone.width = 0.32 * Screen.width;
        mTouchZone.height = 0.5 * Screen.height;
    }
	...
	backgroundRect = background.pixelInset;
...
function ResetJoystick () {
    ...
    gui.pixelInset = mDefaultRect;
    defaultRect = mDefaultRect;
    SetBoundary();
	lastFingerId = -1;
...
function Update () {	
	...
	if (count == 0) {
		...
		for (var i : int = 0; i < count; i++) {
            ...
            else if(touch.phase == TouchPhase.Began && mTouchZone.Contains(touch.position))
            {
			    gui.pixelInset.x = touch.position.x - gui.pixelInset.width * 0.5f;
			    gui.pixelInset.y = touch.position.y - gui.pixelInset.height * 0.5f;

			    defaultRect = gui.pixelInset;

			    background.pixelInset.x = touch.position.x - background.pixelInset.width * 0.5f;
			    background.pixelInset.y = touch.position.y - background.pixelInset.height * 0.5f;
