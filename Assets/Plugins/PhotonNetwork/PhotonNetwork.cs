...
public static class PhotonNetwork
{
    ...
    public static bool offlineMode_inRoom = false;
    ...
    static PhotonNetwork()
    {
        ...
        //PhotonHandler.ShowLog(string.Format("Create network peer:{0}", PhotonHandler.GetProtocol()));
//		Debug.Log ("------------------------NetworkingPeer:"+PhotonHandler.roomConnectType);
        networkingPeer = new NetworkingPeer(photonMono, string.Empty,PhotonHandler.roomConnectType);
...
//				Debug.Log ("------------------------NetworkingPeer:"+PhotonHandler.roomConnectType);
    ...
    public static void Connect(string serverAddress, int port, string appID, string gameVersion)
    {
        ...
        //{
        //    serverAddress=PhotonHandler.GetSverAddress();
        //}
        //serverAddress = PhotonHandler.GetLogicAddr();

		Debug.Log(string.Format("----------------------------master server:{0},port:{1},appid:{2},gamever:{3}",serverAddress,port,appID,gameVersion));
        ...
        PhotonHandler.ShowLog (string.Format ("--------------------------GetLogicAddr:{0}",PhotonHandler.GetLogicAddr()));
        if (string.IsNullOrEmpty(PhotonHandler.GetLogicAddr()))
        {
            PhotonHandler.SetLogicAddr(serverAddress);
        }


        networkingPeer.MasterServerAddress = PhotonHandler.GetLogicAddr()+ ":" + port;
        PhotonHandler.ShowLog(string.Format ("MasterServerAddress:{0}" ,networkingPeer.MasterServerAddress));
    ...
    {
		Debug.Log(string.Format("roomref=>CreateRoom1:{0}",roomName));
    ...
    {
		Debug.Log(string.Format("roomref=>CreateRoom2:{0}",roomName));
    ...
    {
		Debug.Log(string.Format("roomref=>CreateRoom:{0},{1},{2},{3}",roomName,isVisible,isOpen,maxPlayers));
...
//            Debug.LogError("JoinRoom aborted: You can only join a room while not currently connected/connecting to a room.");
    ...
    public static void Destroy(PhotonView targetView)
    {
        if (targetView != null)
        {
            networkingPeer.RemoveInstantiatedGO(targetView.gameObject, false,false);
        }
        else
        {
    ...
    {
        networkingPeer.RemoveInstantiatedGO(targetGo, false,false);
    }
    ...
    internal static void RPC(PhotonView view, string methodName, PhotonPlayer targetPlayer, params object[] parameters)
    {
		...
		{
		   Debug.Log(string.Format("johnlogref,rpc:{0},player:{1}",methodName,targetPlayer.name));
		}
        if (!VerifyCanUseNetwork())
        {
            return;
	...
	//john.setposition
	public static void DoInstantiateYuan(PhotonPlayer photonPlayer,int playerNum)
    {
       networkingPeer.DoInstantiateYuan(photonPlayer,playerNum);
    }
	
	public static void DoClearYuanList()
    {
       networkingPeer.ClearYuanList();
    }
	
	public static void DestroyPlayerObjectsYuan(int playerId)
    {
       networkingPeer.DestroyPlayerObjectsYuan(playerId);
    }
}
