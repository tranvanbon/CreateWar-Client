...
internal class NetworkingPeer : LoadbalancingPeer, IPhotonPeerListener
{
    ...
    public override bool Connect(string serverAddress, string appID)
    {
		...
		{
			PhotonHandler.ShowLog(string.Format("connect:{0},addid:{1}",serverAddress,appID));
		}
...
//            Debug.LogError("ERROR: Cannot connect to Photon while Disconnecting. Connection failed.");
    ...
    private void DisconnectFromMaster()
    {
		if(PhotonHandler.IsShowLog())
		{
			Debug.Log(string.Format("johnlogref DisconnectFromMaster"));
		}
...
//            PhotonNetwork.manuallyAllocatedViewIds = new List<int>();       // filled and easier to replace completely
    ...
    protected internal void LocalCleanupAnythingInstantiated(bool destroyInstantiatedGameObjects)
    {
        ...
        if (destroyInstantiatedGameObjects)
        {
            ...
            {
                this.RemoveInstantiatedGO(go, true,true);
            }
        }
    ...
    private void AddNewPlayer(int ID, PhotonPlayer player)
    {
		if(PhotonHandler.IsShowLog())
		{
			Debug.Log(string.Format("johnlogref AddNewPlayer,id:{0},{1},time:{2}",ID,player.name,System.Environment.TickCount));
		}
...
//            Debug.LogError("Adding player twice: " + ID);
    ...
    private void HandleEventLeave(int actorID)
    {
		...
		if(PhotonHandler.IsShowLog())
		{
			Debug.Log(string.Format("johnlogref HandleEventLeave,actorid:{0}",actorID));
		}
...
//            Debug.LogError("Error: HandleEventLeave for actorID=" + actorID + " has no PhotonPlayer!");
    ...
    private void GameEnteredOnGameServer(OperationResponse operationResponse)
    {
        if (operationResponse.ReturnCode != 0)
        {
			...
			Debug.Log("Non error from disconnnectfromGameServer.");
...
//				Debug.Log("try CreateRoom   "+this.State);
    ...
    {
		Debug.Log(string.Format("roomref=>join,gameid:{0},{1}",gameID,createIfNotExists));
    ...
    public void OnOperationResponse(OperationResponse operationResponse)
    {
		...
		if(PhotonHandler.IsShowLog())
		{
			PhotonHandler.ShowLog(string.Format("OnOperationResponse:{0}",operationResponse.OperationCode));
		}
...
//					         Debug.Log("this.State2 =: " + this.State);
                            if (this.AuthValues != null)
                            {
            ...
            case OperationCode.CreateGame:
                {
                    if (this.State != global::PeerState.Joining)
                    {
				        ...
				        PhotonHandler.ShowLog(string.Format("----------------------------------------recv create game address1,{0}",this.mGameserver));
				        if(PhotonHandler.IsGetLogicSvr())
				        {
					
					        this.mGameserver=PhotonHandler.GetRoomSvr();
            ...
            case OperationCode.JoinGame:
                {
                    if (this.State != global::PeerState.Joining)
                    {
                        ...
                        this.mGameserver = (string)operationResponse[ParameterCode.Address];
				          PhotonHandler.ShowLog(string.Format("recv join game address1,{0}",this.mGameserver));
                        this.DisconnectFromMaster();
                    }
                    else
                    {
            ...
            case OperationCode.JoinRandomGame:
                {
                    ...
                    this.mRoomToGetInto.name = gameID;
                    this.mGameserver = (string)operationResponse[ParameterCode.Address];
			          PhotonHandler.ShowLog(string.Format("recv JoinRandomGame address1,{0}",this.mGameserver));
                    this.DisconnectFromMaster();
                    break;
                }
...
//		Debug.Log("thisStatusCode =: " + statusCode);
        switch (statusCode)
        {
...
//				    Debug.Log("this.State =: " + this.State);
                    PhotonHandler.ShowLog("Connected to gameserver.");
                }
                else
                {
                    if (this.DebugOut >= DebugLevel.ALL)
                    ...
                    PhotonHandler.ShowLog("Connected to masterserver.");
                }
            ...
            case StatusCode.Disconnect:
                if (this.State == global::PeerState.DisconnectingFromMasterserver)
                {
                    ...
                    {
					     PhotonHandler.ShowLog(string.Format("connect when dis,{0}",this.mGameserver));
                ...
                else if (this.State == global::PeerState.DisconnectingFromGameserver)
                {
                    ...
                    {
					    PhotonHandler.ShowLog(string.Format("connect mast when dis,{0}",this.mGameserver));
                ...
                PhotonHandler.ShowLog("networkpeer disconnect");
			    //PhotonHandler.SetViewNull();
                break;
...
//            case StatusCode.DisconnectByServerLogic:
    ...
    {  
		try{
         //Profiler.BeginSample("OnEvent");
        if (this.DebugOut >= DebugLevel.INFO)
        {
            this.DebugReturn(DebugLevel.INFO, string.Format("johnlogref OnEvent: {0}", photonEvent.ToString()));
        }
        ...
        if (photonEvent.Parameters.ContainsKey(ParameterCode.ActorNr))
        {
		    ...
		    if(PhotonHandler.IsShowLog())
			{
				Debug.Log(string.Format("johnlogref OnEvent:{0}",photonEvent.Code));
        ...
        switch (photonEvent.Code)
        {
            ...
            case PunEvent.RPC:
                //ts: each event now contains a single RPC. execute this
			        	...
			        	string inMethodName;
						Hashtable rpcData=(Hashtable)photonEvent[ParameterCode.Data];
...
//				                Debug.LogError("Could not find RPC with index: " + rpcIndex + ". Going to ignore! Check PhotonServerSettings.RpcList");
				 ...
				 if(inMethodName=="SynPositiontable"||(originatingPlayer.isMasterClient||listJionPlayer.ContainsKey (originatingPlayer.ID)))
				{
					 if(PhotonHandler.IsShowLog())
					{
						//Debug.Log(string.Format("johnlogref rpc synpositiontable"));
					}
            ...
            case PunEvent.SendSerializeReliable:
               ...
               if(originatingPlayer.isMasterClient||listJionPlayer.ContainsKey (originatingPlayer.ID)){
            ...
            case PunEvent.Instantiation:
				...
				if(PhotonHandler.IsShowLog())
				{
					Debug.Log("johnlogref PunEvent.Instantiation");
				}
				 Hashtable evDatam = (Hashtable)photonEvent[ParameterCode.Data];
			   string prefabName =(string)evDatam[(byte)0];
			if(originatingPlayer!=null)
			{
                if (originatingPlayer.ID!=PhotonNetwork.player.ID && (prefabName == "ZhanShi"||prefabName == "Fashi"||prefabName == "YouXia"||prefabName == "SoulPet1"||prefabName == "SoulPet2"||prefabName == "SoulPet3"||prefabName == "SoulPet4"||prefabName == "SoulPet5"))//限制显示人数
                {   
				 if(prefabName == "ZhanShi"||prefabName == "Fashi"||prefabName == "YouXia"){
				   if (listMyPlayer.ContainsKey(originatingPlayer.ID))//储存玩家信息
                      listMyPlayer[originatingPlayer.ID] = (Hashtable)photonEvent[ParameterCode.Data];
                  else
                      listMyPlayer.Add(originatingPlayer.ID,(Hashtable)photonEvent[ParameterCode.Data]);
			       }
				 else if(prefabName == "SoulPet1"||prefabName == "SoulPet2"||prefabName == "SoulPet3"||prefabName == "SoulPet4"||prefabName == "SoulPet5"){
				   if (listPet.ContainsKey(originatingPlayer.ID))//储存玩家信息
                      listPet[originatingPlayer.ID] = (Hashtable)photonEvent[ParameterCode.Data];
                  else
                      listPet.Add(originatingPlayer.ID,(Hashtable)photonEvent[ParameterCode.Data]);
			       }
		    	}
			    else
...
//                    Debug.LogError("Can't execute received Destroy request for view ID=" + instantiationId + " as GO can't be foudn. From player/actorNr: " + actorNr + " goToDestroyLocally=" + goToDestroyLocally + "  originating Player=" + originatingPlayer);
                ...
                {
                    this.RemoveInstantiatedGO(goToDestroyLocally, true,true);
                }
...
//                Debug.LogError("Error. Unhandled event: " + photonEvent);
	...
	public void ClearYuanList()
	{
		listJionPet.Clear ();
		listJionPlayer.Clear ();
		listMyPlayer.Clear ();
		listPet.Clear ();
	}
    #endregion
...
//                Debug.LogError("ERROR You have missing MonoBehaviours on your gameobjects!");
	...
	Dictionary<int, Hashtable> listPet = new Dictionary<int, Hashtable>();
	Dictionary<int, Hashtable> listMyPlayer = new Dictionary<int, Hashtable>();
    Dictionary<int, PhotonPlayer> listJionPlayer = new Dictionary<int, PhotonPlayer>();
    Dictionary<int, PhotonPlayer> listJionPet = new Dictionary<int, PhotonPlayer>();
//    List<int> listJionView = new List<int>();
//    List<int> listMyView = new List<int>();

    internal GameObject DoInstantiateYuan(PhotonPlayer photonPlayer,  int playerNum)
    {     
	     
        if (listMyPlayer.ContainsKey(photonPlayer.ID) && !listJionPlayer.ContainsKey(photonPlayer.ID) && listJionPlayer.Count <= playerNum)
        {
           	  listJionPlayer.Add(photonPlayer.ID,photonPlayer);
			  GameObject obj = DoInstantiate(listMyPlayer[photonPlayer.ID], photonPlayer, null);
//                obj.transform.position = mPos;
                obj.SendMessage("SetAttr", photonPlayer, SendMessageOptions.DontRequireReceiver);
//			  return obj;

	        if (listPet.ContainsKey(photonPlayer.ID) && !listJionPet.ContainsKey(photonPlayer.ID) && listJionPet.Count <= playerNum)
            {
           	  listJionPet.Add(photonPlayer.ID,photonPlayer);
			  GameObject pet = DoInstantiate(listPet[photonPlayer.ID], photonPlayer, null);
//                pet.transform.position = mPos;
                pet.SendMessage("Settargetm", obj.transform, SendMessageOptions.DontRequireReceiver);
//			  return obj;
		    }
		}
		return null;
    ...
    public void DestroyPlayerObjectsYuan(int playerId)
    {
        DestroyPlayerObjects(playerId, true);
        if (listJionPlayer.ContainsKey(playerId))
            listJionPlayer.Remove(playerId);
        if (listJionPet.ContainsKey(playerId))
            listJionPet.Remove(playerId);
    }
    ...
    internal GameObject DoInstantiate(Hashtable evData, PhotonPlayer photonPlayer, GameObject resourceGameObject)
    {
		if(PhotonHandler.IsShowLog())
		{
			Debug.Log(string.Format("johnlogref DoInstantiate,plyaer:{0},id:{1}",photonPlayer.name,photonPlayer.ID));
		}
...
//            Debug.LogError(string.Format("DoInstantiate re-defines a GameObject. Destroying old entry! New: '{0}' (instantiationID: {1}) Old: {3}. PhotonViews on old: {4}. instantiatedObjects.Count: {2}. PhotonNetwork.lastUsedViewSubId: {5} PhotonNetwork.lastUsedViewSubIdStatic: {6} this.photonViewList.Count {7}.)", go, instantiationId, this.instantiatedObjects.Count, knownGo, pvaInfo, PhotonNetwork.lastUsedViewSubId, PhotonNetwork.lastUsedViewSubIdStatic, this.photonViewList.Count));
            ...
            this.RemoveInstantiatedGO(knownGo, true,true);
        }
		...
		{
			this.instantiatedObjects[instantiationId]=go;
		}
        // Send mono event
    ...
    public void RemoveAllInstantiatedObjects()
    {
        ...
        for (int index = 0; index < instantiatedGoArray.Length; index++)
        {
            ...
            this.RemoveInstantiatedGO(go, false,true);
        }
...
//            Debug.LogError("Failed to Destroy objects of playerId: " + playerId);
        ...
        {
            this.RemoveInstantiatedGO(gameObject, true,true);
        }
    }
    ...
    public void RemoveInstantiatedGO(GameObject go, bool localOnly,bool keepself)
    {
        if (go == null)
        {
...
//        }
    ...
    public void RegisterPhotonView(PhotonView netView)
    {
		if(PhotonHandler.IsShowLog())
		{
			Debug.Log(string.Format("johnlogref RegisterPhotonView view:{0},{1},time:{2}",netView.viewID,netView.name,System.Environment.TickCount));
		}
...
//                Debug.LogError(string.Format("PhotonView ID duplicate found: {0}. New: {1} old: {2}. Maybe one wasn't destroyed on scene load?! Check for 'DontDestroyOnLoad'. Destroying old entry, adding new.", netView.viewID, netView, photonViewList[netView.viewID]));
            ...
            this.RemoveInstantiatedGO(photonViewList[netView.viewID].gameObject, true,true);
        }
		...
		{
			this.photonViewList[netView.viewID]=netView;
		}
...
//		if(PhotonHandler.GetSelfView())
//		{
//		    PhotonView t_View=PhotonHandler.GetSelfView();
//			this.RPC(PhotonHandler.GetSelfView(),"SynPositiontable",PhotonTargets.All,PhotonHandler.GetPlayerId(),PhotonHandler.GetPositionId());
//			//t_View.RPC("SynPositiontable",);
//			//this->RPC(PhotonHandler.GetSelfView(),"SynPositiontable",PhotonNetwork.player,"");
//			//this->RPC(PhotonHandler.GetSelfView(),"SynPositiontable",PhotonTargets.All,PhotonHandler.GetPositionId());
//			//PhotonView.RPC(
//		}
//		
    }
...
//            Debug.LogError("Cannot remove cached RPCs on a PhotonView thats not ours! " + view.owner + " scene: " + view.isSceneView);
    ...
    internal void RPC(PhotonView view, string methodName, PhotonPlayer player, params object[] parameters)
    {
		...
		if(PhotonHandler.IsShowLog())
		{
		   Debug.Log(string.Format("johnlogref rpcref2:{0},player:{1},viewname:{2}",methodName,player.name,view.name));
		}	
...
//            Debug.LogError("Illegal view ID:" + view.viewID + " method: " + methodName + " GO:" + view.gameObject.name);
    ...
    internal void RPC(PhotonView view, string methodName, PhotonTargets target, params object[] parameters)
    {
		...
		if(PhotonHandler.IsShowLog())
		{
			Debug.Log(string.Format("johnlogref rpcref3:{0},viewname:{1},count{2}",methodName,view.name,parameters.Length));
		}
		
		//PhotonHandler.SetSelfVilw(methodName,view,parameters);
...
//            Debug.LogError("Error: PhotonNetwork.SetReceivingEnabled was called with an illegal group number: " + group + ". The group number should be at least 1.");
    ...
    public void RunViewUpdate()
    {
        ...
        foreach (KeyValuePair<int, Hashtable> kvp in dataPerGroupReliable)
        {
			...
			//			sb.Length=0;
			//foreach(int item in listJionPlayer.Keys)
			//{
    ...
    private Hashtable OnSerializeWrite(PhotonView view)
    {
		...
		if(PhotonHandler.IsWritelDelay())
		{
			if(!view.IsWriteViewData())
			{
				return null;
			}
		}
		
		if(PhotonHandler.IsNotWriteSerial())
		{
			return null;
		}
...
//            Debug.LogError("Observed type is not serializable: " + view.observed.GetType());
    ...
    private void OnSerializeRead(Hashtable data, PhotonPlayer sender, int networkTime, short correctPrefix)
    {
		...
		if(PhotonHandler.IsNotReadSerial())
		{
			return;
		}
		...
		if(PhotonHandler.IsShowLog())
		{
			Debug.Log(string.Format("OnSerializeRead,playername:{0}",sender.name));
		}
